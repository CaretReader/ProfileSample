...
pub const BASE_URL: &str = "https://caretreader.github.io/Sample/";
...
pub fn get_filtered_url(filters: Vec<Filter>, page: i32) -> String {
	...
	if is_searching {
		...
		url.push_str(format!("manga-list-{}-{}-{}-p{}.html", genre, status, sort, page).as_str());
	}
	url
}
...
pub fn get_manga_list(html: Node) -> Result<MangaPageResult> {
	...
	for item in html.select("div.mh-item").array() {
		...
		let id = title_node
			...
			.replace('/', "")
			.replace("bz", "");
		let mut cover = manga_item.select("img.mh-cover").attr("src").read();
		cover = format!("{}{}", BASE_URL, cover);
		let title = title_node.attr("title").read();
		let url = format!("{}{}bz.html", BASE_URL, id);

		let status_str = manga_item.select("span").text().read();
		let status = match status_str.as_str() {
			"Image" => MangaStatus::Ongoing,
			"Photo" => MangaStatus::Completed,
...
pub fn get_manga_details(html: Node, id: String) -> Result<Manga> {
	...
	let mut cover = html.select("img.detail-info-cover").attr("src").read();
	cover = format!("{}{}", BASE_URL, cover);
...
pub fn get_chapter_list(html: Node) -> Result<Vec<Chapter>> {
		...
		.enumerate()
	{
		...
		let url = format!("{}m{}.html", BASE_URL, id);
...
pub fn get_page_list(_url: String) -> Result<Vec<Page>> {
	...
	loop {
		...
		urls.push("https://caretreader.github.io/Sample/photo/1.jpg".to_string());
		urls.push("https://caretreader.github.io/Sample/photo/2.jpg".to_string());
		urls.push("https://caretreader.github.io/Sample/photo/3.jpg".to_string());
		urls.push("https://caretreader.github.io/Sample/photo/4.jpg".to_string());
		urls.push("https://caretreader.github.io/Sample/photo/5.jpg".to_string());
		urls.push("https://caretreader.github.io/Sample/photo/6.jpg".to_string());
...
fn decode(encoded: String) -> Vec<String> {
	...
	for item in pages {
		...
		aidoku::prelude::println!("query: {}", query);
